...
def parse_args():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Limit ECMWF to 10 .nc sample and write to seperate .zarr store",
    )
...
def dpird_to_zarr(dry_run: bool = False):
    ...
    if not dpird_path.exists():
        raise FileNotFoundError(
            f"chunk_n_compress.sh should have ran to produce: {dpird_path}"
        )
    ...
    fill_value = resolve_fill_value(
        DPIRD_SPEC["fill_value"], DPIRD_SPEC["fill_value_dtype"]
    )
    ...
    with xr.open_dataset(dpird_path, engine="h5netcdf") as ds:
        ...
        ds.to_zarr(
            out_path, zarr_format=3, encoding=encoding, mode="w", consolidated=False
        )
...
def ecmwf_to_zarr(dry_run: bool = False):
    ...
    if not ecmwf_files or len(ecmwf_files) <= 1:
        raise FileNotFoundError(
            f"Not enough ECMWF files found at {in_root} with pattern {pattern}"
        )
    ...
    fill_value = resolve_fill_value(
        ECMWF_SPEC["fill_value"], ECMWF_SPEC["fill_value_dtype"]
    )
