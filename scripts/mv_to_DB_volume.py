...
def upload_single_file(w: WorkspaceClient, local_path: str, target_path: str):
    ...
    w.files.upload_from(target_path, local_path, overwrite=True, use_parallel=False)
    return target_path
...
def main():
    ...
    databricks_destination_folder = (
        f"/Volumes/{catalog}/{schema}/{volume}/{volume_target_folder}"
    )
    ...
    for zarr_name in zarr_objects:
        ...
        for local_file_path in zarr_source_path.rglob("*"):
            if local_file_path.is_file():
                ...
                db_target_file_path = (
                    f"{databricks_destination_folder}/{rel_path.as_posix()}"
                )
    ...
    print(
        f"Found {len(files_to_upload)} files to upload to {databricks_destination_folder}"
    )
